...
class DimensionProjections:
    ...
    @plot_bubble
    def pca(self, frame: pd.DataFrame, seed: int) -> (pd.DataFrame, pd.DataFrame, str):
    ...
    @plot_bubble
    def tsne(self, frame: pd.DataFrame, seed: int) -> (pd.DataFrame, pd.DataFrame, str):
        """Perform t-SNE dimensionality reduction and visualisation"""
        proj_2d, proj_3d = None, None
        if len(frame.columns) >= 2:
            tsne_2d = TSNE(n_components=2, random_state=seed, n_jobs=self.workers)
            proj_2d = tsne_2d.fit_transform(frame)
        if len(frame.columns) >= 3:
            tsne_3d = TSNE(n_components=3, random_state=seed, n_jobs=self.workers)
            proj_3d = tsne_3d.fit_transform(frame)
        return proj_2d, proj_3d, 't-SNE'

    @plot_bubble
    def umap(self, frame: pd.DataFrame, seed: int) -> (pd.DataFrame, pd.DataFrame, str):
        """Perform UMAP dimensionality reduction and visualisation"""
        proj_2d, proj_3d = None, None
        if len(frame.columns) >= 2:
            umap_2d = UMAP(n_components=2, random_state=seed)
            proj_2d = umap_2d.fit_transform(frame)
        if len(frame.columns) >= 3:
            umap_3d = UMAP(n_components=3, random_state=seed)
