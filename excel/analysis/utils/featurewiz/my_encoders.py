...
from sklearn.base import (  # gives fit_transform method for free
    BaseEstimator,
    TransformerMixin,
)
import collections
...
class Rare_Class_Combiner_Pipe(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        low_counts = (
            pd.DataFrame(X)
            .apply(lambda x: x.value_counts()[(x.value_counts() <= (0.01 * x.shape[0])).values].index)
            .values.ravel()
        )
...
class Groupby_Aggregator(BaseEstimator, TransformerMixin):
    ...
    def get_params(self, deep=True):
        # This is to make it scikit-learn compatible ####
        return {"categoricals": self.categoricals, "aggregates": self.agg_types, "numerics": self.numerics}
    ...
    def fit(self, X, **fit_params):
        ...
        try:
            print(
                'Beware: Potentially creates %d features (some will be dropped due to zero variance)'
        ...
        for i, each_catvar in enumerate(copy_cats):
            ...
            print('    %d features grouped by %s for aggregates %s' % (num_cols_created, each_catvar, self.agg_types))
...
class Ranking_Aggregator(BaseEstimator, TransformerMixin):
    ...
    def get_params(self, deep=True):
        # This is to make it scikit-learn compatible ####
        return {"categoricals": self.categoricals, "aggregates": self.agg_types, "idvars": self.idvars}
    ...
    def fit(self, X, **fit_params):
        ...
        if isinstance(self.idvars, str):
            if self.idvars == 'all':
                ...
                nunique_max = 0.4
                ID_limit_min = max(
                    10, int(nunique_min * (len(X)))
                )  ### X% of rows must be unique for it to be called ID
                ID_limit_max = max(
                    10, int(nunique_max * (len(X)))
                )  ### X% of rows must be unique for it to be called ID
                ls = nunique_train[(nunique_train[0] <= ID_limit_max) & (nunique_train[0] >= ID_limit_min)][
                    'index'
                ].tolist()
                if len(ls) > 0:
                    ...
                    print(
                        '    No ID vars found that metet criteria of being %s-%s nuniques of dataset length. Returning'
                        % (nunique_min, nunique_max)
                    )
                    return self
            else:
        ...
        for i, each_catvar in enumerate(copy_cats):
            ...
            if len(cols_added) > 0:
                ...
                dft_full = dft[[self.idvars, each_catvar] + cols_added].drop_duplicates(
                    subset=[self.idvars, each_catvar], keep='first'
                )
...
def _create_ts_features(df, tscol, verbose=0):
    ...
    try:
        ...
        if tscol + '_hour' in dt_adds:
            ...
            df.loc[:, tscol + '_dayofweek_hour_cross'] = (
                df[tscol + '_dayofweek'] + " " + df[tscol + '_hour'].astype(str)
            )
        ...
        MONTHS = dict(
            zip(range(1, 13), ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
        )
    ...
    def fit(self, X, y, **fit_params):
        ...
        for each_num in self.continuous_vars:
            ...
            if is_numeric_dtype(y) and self.max_number_of_classes > 25:
                clf = DecisionTreeRegressor(criterion='mse', min_samples_leaf=2, max_depth=max_depth, random_state=seed)
            else:
                clf = DecisionTreeClassifier(
                    criterion='entropy', min_samples_leaf=2, max_depth=max_depth, random_state=seed
                )
            try:
        ...
        if self.fitted and y is None:
            ...
            try:
                ...
                for each_target in self.targets:
                    ...
                    if len(self.hier_vars) == 0:
                        self.train[newcol] = (
                            self.train.set_index(['new_' + self.ts_column])
                            .index.map(self.ratios[each_target].get)
                            .fillna(0)
                        )
                    else:
                        self.train[newcol] = (
                            self.train.set_index(self.hier_vars + ['new_' + self.ts_column])
                            .index.map(self.ratios[each_target].get)
                            .fillna(0)
                        )
...
class TS_Trend_Seasonality_Transformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        try:
            ...
            return self
        self.ratios = (
            self.train.groupby([self.categorical_var, self.ts_column]).sum()
            / self.train.groupby([self.ts_column]).sum()
        )[self.targets].to_dict()
        self.fitted = True
        return self
    ...
    def transform(self, X, y=None):
        ...
        if self.fitted and y is None:
            ...
            numtrans = Numeric_Transformer(ts_columns=[self.ts_column])
            new_columns = left_subtract(
                self.X_transformed.columns.tolist(), self.ratio_col_adds + self.percent_col_adds
            )
        ...
        try:
            for each_target in self.targets:
                self.train[each_target + '_' + self.categorical_var + '_trend'] = self.train.set_index(
                    [self.categorical_var, self.ts_column]
                ).index.map(self.ratios[each_target].get)
                self.train[each_target + '_' + self.categorical_var + '_seasonality'] = (
                    self.train[each_target] / self.train[each_target + '_' + self.categorical_var + '_trend']
                )
    ...
    def transform(self, X, y=None):
        ...
        if self.time_period == 'daily':
            if self.seasonality == '1year':
                ...
                self.dayofbiyear = X[self.ts_column].dt.dayofyear + 365 * (
                    1 - (X[self.ts_column].dt.year % 2)
                )  # 1 to 730
            ...
            if self.seasonality == '1year':
                ...
                self.dayofbiyear = X[self.ts_column].dt.weekofyear + 52 * (
                    1 - (X[self.ts_column].dt.year % 2)
                )  # 1 to 104
                self.listofyears = [1, 2, 4]
...
class Numeric_Transformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if not (len(self.lis) == 0):
            for everycol in self.lis:
                try:
                    ...
                    print(
                        f'    error converting {everycol} column from string to numeric, deteail : {e}. Continuing...'
                    )
        ...
        print(
            'After converting all date, object and category columns to numeric, shape of data: %s'
            % (X[self.columns].shape,)
        )
